...
def this_main(tool_name, ontonotes5_test_file_path, follow_up_inputs_csv_file_path, violations_csv_file_path):
    ...
    for num in tqdm(range(len(random_source_inputs)), desc='source output'):
        ...
        if len(this_sample) == 0:
            continue
        if this_sample[0] in ["When", "when"]:
            if "," not in this_sample:
                continue
            if this_sample[0] == "When":
                index_when = this_sample.index("When")
            elif this_sample[0] == "when":
                index_when = this_sample.index("when")
            if tool_name == "flair":
            ...
            if this_sample[-1] in punc or this_sample[-1] == "/.":
                this_when_clause = this_sample[index_when:index_comma]
                this_when_clause[0] = this_when_clause[0].lower()
                this_when_clause_tag = list_pos[index_when:index_comma]
                this_follow_input = this_sample[index_comma + 1:-1]
                this_follow_input.append(",")
                this_follow_input.extend(this_when_clause)
                this_follow_input.append(".")
                ...
                this_follow_target.extend(this_when_clause_tag)
                if tool_name == "spacy":
                    this_follow_target.append("PUNCT")
                else:
                    this_follow_target.append(".")
                ...
                this_num += 1
            continue
        if "when" not in this_sample:
            continue
        if this_sample[0] not in ["I", "i"]:
            this_sample[0] = this_sample[0][0].lower() + this_sample[0][1:]
        index_when = this_sample.index("when")
        if this_sample[index_when - 1] in ["is", "are", "'s", "'re", "'re", "was", "were"]:
            continue
        if tool_name == "flair":
            list_pos = flair_out(this_sample)
        elif tool_name == "nltk":
            ...
            list_pos = spacy_out(this_sample)
        this_orignial_when = this_sample[index_when:]
        if "," in this_orignial_when:
            this_index_comma = this_orignial_when.index(",")
            this_when_clause = this_sample[index_when:index_when + this_index_comma]
            this_when_clause_tag = list_pos[index_when:index_when + this_index_comma]
            if this_sample[index_when - 1] == ",":
                this_follow_input = this_when_clause[:]
                this_follow_input.append(",")
                this_follow_input.extend(this_sample[:index_when - 1])
                this_follow_input.extend(this_sample[index_when + this_index_comma:])
                this_follow_input[0] = this_follow_input[0][0].upper() + this_follow_input[0][1:]
                this_follow_target = this_when_clause_tag[:]
                if tool_name == "spacy":
                    this_follow_target.append("PUNCT")
                else:
                    this_follow_target.append(",")
                this_follow_target.extend(list_pos[:index_when - 1])
                this_follow_target.extend(list_pos[index_when + this_index_comma:])
                follow_input.update({this_num: this_follow_input})
                follow_input_source.update({this_num: this_sample})
                follow_target.update({this_num: this_follow_target})
                this_num += 1
                ...
                this_follow_input = this_when_clause[:]
                this_follow_input.append(",")
                this_follow_input.extend(this_sample[:index_when])
                this_follow_input.extend(this_sample[index_when + this_index_comma:])
                this_follow_input[0] = this_follow_input[0][0].upper() + this_follow_input[0][1:]
                this_follow_target = this_when_clause_tag[:]
                if tool_name == "spacy":
                    ...
                    this_follow_target.append(",")
                this_follow_target.extend(list_pos[:index_when])
                this_follow_target.extend(list_pos[index_when + this_index_comma:])
                follow_input.update({this_num: this_follow_input})
                follow_input_source.update({this_num: this_sample})
                follow_target.update({this_num: this_follow_target})
            ...
            if this_sample[-1] in punc or this_sample[-1] == "/.":
                if this_sample[index_when - 1] == ",":
                    this_when_clause = this_sample[index_when:-1]
                    this_when_clause_tag = list_pos[index_when:-1]
                    this_follow_input = this_when_clause[:]
                    this_follow_input.append(",")
                    this_follow_input.extend(this_sample[:index_when - 1])
                    this_follow_input.append(".")
                ...
                if this_sample[index_when - 1] == ",":
                    ...
                    this_when_clause = this_sample[index_when:]
                    this_when_clause_tag = list_pos[index_when:]
                    this_follow_input = this_when_clause[:]
                    this_follow_input.append(",")
                    this_follow_input.extend(this_sample[:index_when])
                    this_follow_input.append(".")
