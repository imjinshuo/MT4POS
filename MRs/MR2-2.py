...
def this_main(tool_name, ontonotes5_test_file_path, follow_up_inputs_csv_file_path, violations_csv_file_path):
    ...
    for num in tqdm(range(len(source_input)), desc='source output'):
        ...
        for i in c:
            if this_sample[-1] in punc or this_sample[-1] == "/.":
                this_source_input = source_input[i]
                if this_source_input[0] not in ["I", "i"]:
                    this_source_input[0] = this_source_input[0][0].lower() + this_source_input[0][1:]
                this_follow_input = this_sample[:-1]
                this_follow_input.append(",")
                this_follow_input.append("and")
                this_follow_input.extend(this_source_input)
                this_follow_input[0] = this_follow_input[0][0].upper() + this_follow_input[0][1:]
                this_follow_target = list_pos[:-1]
                this_ten_follow_input.append(this_follow_input)
            else:
                this_source_input = source_input[i]
                if this_source_input[0] not in ["I", "i"]:
                    this_source_input[0] = this_source_input[0][0].lower() + this_source_input[0][1:]
                this_follow_input = this_sample[:]
                this_follow_input.append(",")
                this_follow_input.append("and")
                this_follow_input.extend(this_source_input)
                this_follow_input[0] = this_follow_input[0][0].upper() + this_follow_input[0][1:]
    ...
    for num in tqdm(range(len(ten_follow)), desc='follow output'):
        ...
        for follow_sent in ten_follow[num]:
            ...
            out_file = False
            if this_follow_output[:follow_target_index[num]] != this_target_tag[:]:
                for this_num in range(follow_target_index[num]):
                    if this_target_tag[this_num] != this_follow_output[this_num]:
